...
export default function Estadisticas({ patients }: EstadisticasProps) {
  ...
  const Summary = () => {
      ...
      const mostCommonCoinfeccion = coInfeccionData.reduce((max, current) => current.value > max.value ? current : max, {name: 'ninguna', value: 0});
      ...
      return (
        <div className="bg-blue-50 border border-blue-200 text-blue-800 p-4 rounded-lg mb-8 space-y-1">
            ...
            <p className="text-sm">
                ...
                {mostCommonCoinfeccion.value > 0 && (
                    <>
                        {' '}La co-infección más reportada es <span className="font-bold">{mostCommonCoinfeccion.name}</span>.
                    </>
                )}
            </p>
            <p className="text-sm">
